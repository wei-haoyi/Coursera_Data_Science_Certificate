...
colleting the right data and the data is relvant to the question you are trying to answer.
...
* interpretable, simple, accurate, fast ( to train and test), scalable ( easy to apply to a large dataset.)

### Prediciton is about accuracy tradeoffs
...
### In sample and out of smaple error
...
Key ides
1. out of smaple error is what you care about
2. in sample error < out of smaple error
3. the reason is overfitting
* matching your algorithm to the data you have.

Why more sophscated alogrithm yield higher out of sample error"
* overfitting
...
### Prdiction study design
1. Define your error rate
2. Split data into :
* training, testing, validation (optional)
3. On thetraining set pick features
...
* apply 1* to validation


#### Avioid small sample sizes. 
...
#### some principles to remember
 set the test/validaiton set aside and don't look at it
...
positive predictive value = TP/TP+FP 测出有病的有多少是真有病 precision
negative predictive value = TN/FN+TN 测出没病的有多少是真没病
...
Root mean squared error (RMSE) square root of the previous
A good measure of the error of the continuos data.
...
* but your predictions are ofoten quantitative
-probability of being alive

* the cutoff you choose gives different results

senstivity( recall) on the y axis (true positive rate)
(1-specificity) on the x axis (false positive rate)
...
* for k-fold cross validation
 ...
 + smaller k = more bias, less variamce
* random sampling must be done without replacement
* random sampling with replacement is the bootstrap
+ underestimates of the erorr
...
##  Week 8.2 Caret package

### Caret functionality

* some preprocessing (cleanihng)
  + preProcess
* Data spliting
  + createDataPartition
  + createResample
...
### Basic preprocessing the predicator
...
Training and test must be preocssed in the same way
...
* We should pick this cobination to capture the "most information" possible
* benefits
+ reduced number of predictors
+ reduced noise (due to averaging)

You have multipvaraite variables,
* find a new set of multivariate variables that are uncorrelated and explain as much variance as possible.
* if you put all the variables together in one matrix, find the best matrix created with fewer variables that explains the orginal data.
...
You have multivariate variables X1, ..., Xn, so X11,....X1m
* Find a new set of multivariate variables that are uncorrelated and explain as much variance as possible.
...
* For more info see exploratory data analysis


## Week 8.3 Predictin with trees, Random Forests, & Model Based predicitons
...
* start with all varaibles in one group
* Find the vvariable/split that best separates the outcomes
* divide the data into two groups ("leaves") on that split ("node)
* whithin each split, find the best variable/split that separates the outcomes
...
* Note that three are multiple tree building options in R both in the caret package - party ,rpart and out of the caret package - tree
...
Note
random forest are usually one of the two top performing algorithms along with boosting in prediction contests.
...
Boosting in R
Boosting can be used wiht any subset of classifiers
...
arereasonably accurate on real problems
...
make additional assumptions about the data
 ...
 linear discriminant analysis assumes ... is multivariate gaussian with same covariances
 quadratic discrimant analysis assume .. is multivariate gaussian with different covariance
 model based prediciton assume more complicate version for the covariance matrix
 naive abyes assumes independence between features for model building.
